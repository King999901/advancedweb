...
function Teacher() {
  ...
  return (
    <motion.div
      id="teacher"
      className="w-[100%] h-screen bg-gradient-to-tr from-purple-50 to-pink-50 flex items-center justify-center overflow-hidden"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      ...
      >
        ...
        >
          ...
          <div className="text-right">
            <motion.h1 className="text-4xl font-bold mb-2 text-purple-700">
              {teacherData.name}
            </motion.h1>
        ...
        >
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {["web", "research"].map((cat, idx) => (
              <div key={idx}>
                <h3 className="text-xl font-medium mb-2 text-right">
                  {cat === "web" ? "وب" : "تحقیق و طراحی"}
                </h3>
        ...
        <motion.section
          className="mt-6 bg-pink-50 rounded-lg p-4"
          initial={{ y: 50, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
        ...
        {/* Experience Section */}
        <motion.section
          className="mt-6 bg-yellow-50 rounded-lg p-4"
          initial={{ y: 50, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
        ...
        >
          ...
          {teacherData.experience.map((exp, index) => (
            <motion.div
              key={index}
              className="mb-4 p-4 bg-white rounded-lg hover:bg-gray-100 transition-colors text-right"
              whileHover={{ scale: 1.02 }}
              transition={{ duration: 0.3 }}
            >
